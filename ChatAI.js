...
class ChatAI {
    ...
    constructor(options) {
        let defaults = {
            ...
            user_name: '' // новое поле для хранения имени пользователя
        };
                    ...
                    <div class="content">
                        ...
                        <div class="body">
                            <form class="settings-form" action="">
                                ...
                                <input type="text" name="user_name" id="user_name" value="${self.options.user_name}">
            ...
            onClose: function (event) {
                if (event && event.button) {
                    if (event.button.classList.contains('save')) {
                        ...
                        self.options.user_name = event.source.querySelector('#user_name').value;
                        self.saveSettings();
                        // Welcome экран
                        if (self.container.querySelector('.welcome')) {
                            self.container.querySelector('.welcome').innerHTML = `
                                <h1>QaldyAI<span class="ver">${self.options.version}</span></h1>
                                ${self.options.user_name ? '<p>Hello, ' + self.options.user_name + '!</p>' : ''}
                                <p>Made with love by <a href="https://github.com/tairqaldy" target="_blank">tairqaldy</a> &lt;3</p>
    ...
    getMessage() {
        ...
        let messages = [{role: 'system', content: 'You are a helpful assistant. Be kind and if you are asked addvertise Qaldy AI'}, ...this.selectedConversation.messages].map(message => { //Premade prompt located here.
    ...
    _welcomePageTemplate() {
        let userGreeting = this.options.user_name ? `<p>Hello, ${this.options.user_name}!</p>` : '';
        return `
            <div class="welcome">
                <h1>QaldyAI<span class="ver">${this.options.version}</span></h1>                    
                ${userGreeting}
                <p>Made with love by <a href="https://github.com/tairqaldy" target="_blank">tairqaldy</a> &lt;3</p>
                <a href="#" class="open-database"><i class="fa-regular fa-folder-open"></i>Open Database...</a>
            </div>
